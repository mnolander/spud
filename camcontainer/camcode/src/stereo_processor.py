...
class StereoProcessor:
    ...
    def rectify_frames(self, frame_left, frame_right, cam_id1, cam_id2):
        ...
        if not isinstance(frame_left, np.ndarray):
            print(f"Warning: Frame from {cam_id1} is not a valid NumPy array!")
            return
        if not isinstance(frame_right, np.ndarray):
            print(f"Warning: Frame from {cam_id2} is not a valid NumPy array!")
            return
        
        # Ensure frames are not empty
        if frame_left.size == 0 or frame_right.size == 0:
            print(f"Warning: Frame from {cam_id1} or {cam_id2} is empty!")
            return
        ...
        # Resize frames to match calibration image size
        if frame_left.shape[:2] != self.image_size:
            print(f"Resizing {cam_id1} from {frame_left.shape[:2]} to {self.image_size}")
            frame_left = cv2.resize(frame_left, self.image_size, interpolation=cv2.INTER_LINEAR)
        if frame_right.shape[:2] != self.image_size:
            print(f"Resizing {cam_id2} from {frame_right.shape[:2]} to {self.image_size}")
