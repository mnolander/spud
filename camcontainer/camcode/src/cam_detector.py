...
import sys
import os
import tf.transformations as tf_transforms 
...
class DetectorThread(threading.Thread):
    ...
    def compute_3d_positions(self):
        ...
        if not left_detections or not right_detections:
            return # No detections on one or both cameras
        ...
        for tag_id, (x, y, z) in zip(matched_ids, points_3d):
            ...
            if success:
                ...
                quaternion = [0.0, 0.0, 0.0, 1.0] # Default quaternion if solvePnP fails
    ...
    def run(self):
        while not self.killswitch.is_set():
            ...
            # Detect tags in the rectified frame
            detections = self.detector.detect(rectified_frame)
            detections_fullres = []

            detected_tags = {} # Store detected tags for 3D position computation
            ...
            # Compute 3D positions if both cameras detect the same tags
            ...
            # Push the rectified frame and detections to the consumer (frame_consumer.py)
