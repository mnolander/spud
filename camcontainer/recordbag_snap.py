...
class FrameConsumer:
    ...
    def run(self):
        IMAGE_CAPTION = 'Snapshot Example: Press <S> to start/stop snapshot, <Enter> to exit'
        KEY_CODE_ENTER = 13
        KEY_CODE_S = ord('s')

        frames = {}
        alive = True
        taking_snapshots = False  # Add a flag to track snapshot state
        bag_filename = time.strftime("%Y%m%d-%H%M%S") + '_snapshot.bag'
        
        # Open the rosbag file once
        with rosbag.Bag(bag_filename, 'w') as bag:
            ...
            while alive:
                ...
                if key == KEY_CODE_ENTER:
                    ...
                    taking_snapshots = not taking_snapshots  # Toggle snapshot state
                    if taking_snapshots:
                        print("Started taking snapshots...")
                    else:
                        print("Stopped taking snapshots.")

                # If snapshotting is enabled, save the frames
                if taking_snapshots and len(cv_images) > 0:
                    ...
                    print(f'Snapshot saved to {bag_filename}')
...
if __name__ == '__main__':
    print_preamble()
    rospy.init_node('multi_camera_snapshot', anonymous=True)
    app = Application()
    app.run()
